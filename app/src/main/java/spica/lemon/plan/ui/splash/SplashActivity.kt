...
class SplashActivity : BindingActivity<ActivitySplashBinding>() {
    ...
    private fun showSpaceObjectsWithAnimation(positionOffset: Float) {
        viewBinding.mars.animate()
            .translationX(0 - (MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.neptune.animate()
            .translationX((MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.moon.animate()
            .translationX((MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.saturn.animate()
            .translationX((MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.venus.animate()
            .translationY(0 - (FAR_DISTANCE - (FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
    ...
    private fun hideVegtablesObjectWithAnimation(positionOffset: Float) {
        ...
        viewBinding.tomato.animate().translationX(0 - ((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.potato.animate().translationX(0 - ((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.onion.animate().translationX(((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.pickel.animate().translationX(((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.carrots.animate().translationY(0 - ((FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.rightHalfCircle.animate().translationY(0 - ((FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
    }
    ...
    private fun hidePlantesObjectsWithAnimation(positionOffset: Float) {
        ...
        viewBinding.leaf.animate().translationX((0 - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.sanawbar.animate().translationX(((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.earsTree.animate().translationX(((MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.upperPurpleRose.animate().translationY(0 - ((FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.purpleRose.animate().translationY(0 - ((FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.pointsLeaf.animate().translationY(0 - ((FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
    }

    private fun animateFragmentObjects(positionOffset: Float) {
        if (findViewById<View>(R.id.two_papers) != null) {
            findViewById<View>(R.id.two_papers).animate()
                .translationY(((MEDIUM_DISTANCE * positionOffset))).setDuration(0).start()
        }
    }

    private fun showVegtablesObjectsWithAnimation(positionOffset: Float) {
        viewBinding.tomato.animate()
            .translationX(0 - (MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.potato.animate()
            .translationX(0 - (MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.onion.animate()
            .translationX((MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.pickel.animate()
            .translationX((MEDIUM_DISTANCE - (MEDIUM_DISTANCE * positionOffset)))
            .setDuration(0).start()
        viewBinding.carrots.animate()
            .translationY(0 - (FAR_DISTANCE - (FAR_DISTANCE * positionOffset)))
            .setDuration(0).start()
